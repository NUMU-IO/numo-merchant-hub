...
export default function ResetPassword() {
  ...
  const inputCls = "h-11 rounded-lg border-border/70 placeholder:text-muted-foreground/40 focus:border-foreground focus:ring-1 focus:ring-foreground/5 transition-colors";

  if (!token) {
    return (
      <div className="min-h-screen auth-page auth-dot-grid flex flex-col items-center justify-center p-4 sm:p-6">
        <div className="w-full max-w-[420px]">
          <div className="bg-background rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.12)] p-7 sm:p-9 auth-enter text-center">
            <div className="flex justify-center mb-4">
              <div className="h-11 w-11 rounded-full bg-destructive/[0.06] flex items-center justify-center">
                <AlertTriangle className="h-5 w-5 text-destructive" />
              </div>
            </div>
            <h1 className="text-xl font-semibold tracking-tight">{t("auth.invalidResetLink", "Invalid or expired reset link")}</h1>
            <p className="mt-2 text-sm text-muted-foreground">{t("auth.requestNewLink", "Please request a new password reset link.")}</p>
        ...
        <div className="bg-background rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.12)] p-7 sm:p-9 auth-enter">
          {success ? (
            <div className="text-center">
              ...
              <h1 className="text-xl font-semibold tracking-tight">{t("auth.passwordReset", "Password Reset")}</h1>
              <p className="mt-2 text-sm text-muted-foreground">{t("auth.canNowLogin", "You can now log in with your new password.")}</p>
            ...
            <div>
              <h1 className="text-xl font-semibold tracking-tight">{t("auth.newPassword", "Set New Password")}</h1>
              <p className="mt-1.5 text-sm text-muted-foreground mb-7">{t("auth.newPasswordDesc", "Enter your new password below.")}</p>

              <form noValidate onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="password" className="text-[13px] font-medium">{t("auth.newPassword", "New Password")}</Label>
                ...
                <div className="space-y-2">
                  <Label htmlFor="confirmPassword" className="text-[13px] font-medium">{t("auth.confirmPassword", "Confirm Password")}</Label>
                ...
                <Button type="submit" className="w-full h-11 text-sm font-semibold rounded-lg mt-1" disabled={loading || !password || !confirmPassword}>
