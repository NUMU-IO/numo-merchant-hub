...
import {
  Bell, Check, ChevronDown, Globe, LogOut, Moon, Plus, Search, Settings, Sun, User, ExternalLink,
} from "lucide-react";
...
const AppHeader = () => {
  ...
  const openStore = useCallback(() => {
    ...
    const url = currentStore.store_url || (currentStore.subdomain ? getStoreUrl(currentStore.subdomain) : null);
          ...
          {stores.length > 0 && (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56 rounded-xl">
                ...
                {/* Open store link */}
                <DropdownMenuItem onClick={openStore} className="gap-2 rounded-lg">
                  <ExternalLink className="h-3.5 w-3.5" />
                  <span className="text-sm">{language === "ar" ? "فتح المتجر" : "Open store"}</span>
                </DropdownMenuItem>
                {/* Create new store */}
