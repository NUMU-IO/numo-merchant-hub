...
function App() {
  ...
  const [firstTime, setFirstTime] = useA(false);
  const [sheet, setSheet] = useA(null); // "more" | "add" | null
  ...
  const navigate = (k) => { setRoute(k); setSheet(null); };

  const SCREENS = {
    dashboard: Dashboard, orders: Orders, products: Products, customers: Customers,
    store: OnlineStore, marketing: Marketing, analytics: Analytics,
    finance: Finance, cod: COD, logistics: Logistics, settings: Settings,
  };
  const top = baseKey(route);
  const sub = (route.split(":")[1]) || null;
  const Screen = SCREENS[top] || Dashboard;
  ...
  const Controls = (
    <div style={{ position: "fixed", bottom: 18, insetInlineStart: "50%", transform: "translateX(-50%)", zIndex: 100,
      ...
      <button className={"chip" + (firstTime ? " on" : "")} style={{ height: 34 }} onClick={() => { setFirstTime(f => !f); setRoute("dashboard"); }} title="Toggle first-time merchant (zero data)">
        <Icon name="seedling" size={16} />{firstTime ? "First-time" : "Full data"}
      </button>
    </div>
  );
  ...
  if (!authed) {
      ...
      <div style={{ height: "100%", background: "var(--surface-2)", display: "grid", placeItems: "center" }}>
        <div className="device" style={{ borderInline: "1px solid var(--border)" }}>
          ...
          <div className="m-scroll" key={route + dir}>
            <Screen ar={ar} go={navigate} mobile={true} sub={sub} firstTime={firstTime} />
          </div>
          <MobileNav route={route} setRoute={navigate} ar={ar} onMore={() => setSheet("more")} onAdd={() => setSheet("add")} />
          {sheet === "more" && <MoreSheet route={route} setRoute={navigate} ar={ar} onClose={() => setSheet(null)} />}
          {sheet === "add" && <AddSheet ar={ar} go={navigate} onClose={() => setSheet(null)} />}
        </div>
      </div>
    );
  } else {
    body = (
      <div className="app-shell">
        <Sidebar route={route} setRoute={navigate} ar={ar} />
        <div className="app-main">
          <Header ar={ar} setAr={setAr} dark={dark} setDark={setDark} />
          <div className="app-scroll" key={route + dir}><Screen ar={ar} go={navigate} mobile={false} sub={sub} firstTime={firstTime} /></div>
        </div>
      </div>
    );
  }
